...
const HARD_SIZE_CAP_LOC = 800;
const SOFT_SIZE_WARN_LOC = 300;
const CHRISTMAS_TREE_FILE_COUNT = 8;
const PACKAGE_SPREAD_LIMIT = 3;
const MIN_ASSERTIONS_PER_TEST = 2;
const PURE_ADDITION_ADDS_THRESHOLD = 200;
...
const BUILD_OUTPUT_PATTERNS: readonly RegExp[] = [
  /^packages\/[^/]+\/dist\//,
  /^dist\//,
  /\.tsbuildinfo$/,
];

const RELEASE_MANIFEST_PATH = '.github/release-please-manifest.json';
const PACKAGE_CHANGELOG_RE = /^packages\/[^/]+\/CHANGELOG\.md$/;
const PACKAGE_MANIFEST_RE = /^packages\/[^/]+\/package\.json$/;
const PROD_FILE_RE = /^packages\/[^/]+\/src\/.+\.ts$/;
const TEST_FILE_RE = /^packages\/[^/]+\/test\/.+\.test\.ts$/;
const PACKAGE_DIR_RE = /^packages\/([^/]+)\//;
...
function checkPackageSpread(files: readonly string[]): void {
  ...
  if (packages.size > PACKAGE_SPREAD_LIMIT) {
    // Array.from avoids a Danger runtime quirk where `[...set]` spread
    // yields an empty array even when `set.size` is non-zero.
    const list = Array.from(packages)
      .map((name) => `- \`packages/${name}/\``)
      .join('\n');
    warn(
      `PR touches ${packages.size} packages (threshold: ${PACKAGE_SPREAD_LIMIT}). Cross-package churn often means a refactor that should be split. Packages:\n${list}`,
    );
  }
}
...
async function checkTestToProdRatio(): Promise<void> {
  ...
  if (prodLines > 50) {
    fail(
      'Production files under `packages/*/src/` changed with no matching test file added or modified. Add a test, or a `Skip-tests-justified: <reason>` trailer to justify.',
    );
  } else if (prodLines > 0) {
    warn(
      'Production files under `packages/*/src/` changed with no matching test file. Add a test, or a `Skip-tests-justified: <reason>` trailer to justify.',
    );
  }
}
...
function readManifest(ref: string | null, path: string): PackageManifest | null {
  try {
    const text =
      ref === null ? runGit(['show', `HEAD:${path}`]) : runGit(['show', `${ref}:${path}`]);
...
interface DepDelta {
  ...
  readonly added: ReadonlyArray<{ label: string; names: string[] }>;
}

function diffManifest(path: string, base: string): DepDelta | null {
  const before = readManifest(base, path);
  const after = readManifest(null, path);
  ...
  const added = groups
    .map((group) => ({ label: group.label, names: addedKeys(group.before, group.after) }))
    .filter((entry) => entry.names.length > 0);
  return { path, depsChanged, added };
}

function checkDependencyDelta(base: string, files: readonly string[]): void {
  const manifestPaths = files.filter(
    (path) => path === 'package.json' || PACKAGE_MANIFEST_RE.test(path),
  );
  if (manifestPaths.length === 0) return;
  const deltas = manifestPaths
    .map((path) => diffManifest(path, base))
    .filter((delta): delta is DepDelta => delta !== null);
  const anyDepsChanged = deltas.some((delta) => delta.depsChanged);
  const lockChanged = files.includes('pnpm-lock.yaml');
  if (anyDepsChanged && !lockChanged) {
    fail(
      'Package manifests changed dependencies without a corresponding `pnpm-lock.yaml` update. Run `pnpm install`.',
    );
  }
  for (const delta of deltas) {
    for (const group of delta.added) {
      const list = group.names.map((name) => `\`${name}\``).join(', ');
      warn(
        `New ${group.label} in \`${delta.path}\`: ${list}. Verify the packages and versions before merging.`,
      );
    }
  }
}
...
function packageVersionChanged(path: string, base: string): boolean {
  const before = readManifest(base, path);
  const after = readManifest(null, path);
...
function checkReleaseManagedFiles(base: string, files: readonly string[]): void {
  ...
  if (files.includes(RELEASE_MANIFEST_PATH)) touched.push(RELEASE_MANIFEST_PATH);
  for (const path of files) {
    if (PACKAGE_CHANGELOG_RE.test(path)) touched.push(path);
  }
  // `packages/*/package.json` is partly release-managed: release-please owns
  // the `version` field (and workspace-dep bumps via the `node-workspace`
  // plugin). Edits to other fields are human-owned, so only fail when the
  // version actually changed.
  for (const path of files) {
    if (!PACKAGE_MANIFEST_RE.test(path)) continue;
    if (packageVersionChanged(path, base)) touched.push(path);
  }
